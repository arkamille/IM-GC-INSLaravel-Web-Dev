...
<?php
echo "<h3> Soal No 1 Greetings </h3>"; /*Soal No 1 Greetings Buatlah sebuah function greetings() yang menerima satu parameter berupa string. 
contoh: greetings("abduh"); Output: "Halo Abduh, Selamat Datang di Sanbercode!" */
// Code function di sini // Hapus komentar untuk menjalankan code!
function greetings($you){
    echo "Halo ".$you.", Selamat Datang di Sanbercode!<br>";
}
greetings("Bagas");  greetings("Wahyu");  greetings("nama peserta");
echo "<br>"; echo "<h3>Soal No 2 Reverse String</h3>"; /*Soal No 2 Reverse String 
Buatlah sebuah function reverseString() untuk mengubah string berikut menjadi kebalikannya menggunakan function dan looping (for/while/do while).
Function reverseString menerima satu parameter berupa string. 
NB: DILARANG menggunakan built-in function PHP sepert strrev(), HANYA gunakan LOOPING! reverseString("abdul"); Output: ludba
*/
// Code function di sini // Hapus komentar di bawah ini untuk jalankan Code
function reverseString($string){
    $j;//MENGHITUNG PANJANG STRING DENGAN BATAS 1000 KARAKTER
    for($i=0; $i<1000; $i++){
        if( $string[$i] <> null){
            $j++;                 
        }        
    }//MENAMPILKAN STRING PERKARAKTER DIMULAI DARI AKHIR STRING
    while($j>=0){
        echo $string[$j];
        $j--;
    }
...
reverseString("nama peserta");  reverseString("Sanbercode");  reverseString("We Are Sanbers Developers"); echo "<br>"; 
echo "<h3>Soal No 3 Palindrome </h3>"; /*Soal No 3 Palindrome 
Buatlah sebuah function yang menerima parameter berupa string yang mengecek apakah string tersebut sebuah palindrome atau bukan. 
Palindrome adalah sebuah kata atau kalimat yang jika dibalik akan memberikan kata yang sama contohnya: katak, civic. 
Jika string tersebut palindrome maka akan mengembalikan nilai true, sedangkan jika bukan palindrome akan mengembalikan false. 
NB: Contoh: palindrome("katak") => output : "true" palindrome("jambu") => output : "false" 
...
function palindrome($string){
    ...
    }//MENYIMPAN STRING PERKARAKTER DIMULAI DARI AKHIR STRING
    $palindrome; $w; while($j>=0){        
        $palindrome[($w-1)] = $string[$j];
        $j--; $w++;
    }//PALINDROME CONDITION TRUE OR FALSE PER KARAKTER STRING
    for($i=0; $i<1000; $i++){
        if( $string[$i] <> null){
            if ($palindrome[$i] == $string[$i]) $tf = "true";
            else $tf = "false";
            $j++;                 
        }
    }
